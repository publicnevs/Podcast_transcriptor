...
# With Playwright/Chromium for JS-rendered page scraping (heavy image):
docker-compose build --build-arg INSTALL_BROWSER=true
docker-compose up -d
# Then enable "JavaScript-Seiten rendern" in Settings → Scraping & Audio
```
...
- **`transcriber.py`** — Gemini API calls. Models are env-overridable (`GEMINI_FLASH_MODEL`, `GEMINI_PRO_MODEL`, `GEMINI_LITE_MODEL`, plus `GEMINI_DIGEST_MODEL` which defaults to the Pro model), defaulting to `gemini-2.5-flash` / `gemini-2.5-pro` / `gemini-2.5-flash-lite`. **Cost note**: `GEMINI_LITE_MODEL` must be explicitly set in `.env` — without it the Lite path silently falls back to Flash, so no cost savings. Three paths:
...
- **`statistics.html`** — Feed health dashboard: per-feed poll history from `feed_check_log`, episode counts by type, and a sparkline of activity. Read-only; served at `/statistics`.
...
- **`sw.js`** — Service Worker (cache `podscribe-v15`); caches the app shell incl. `icons.js` for offline reading, and **skips** `/audio` and `/export` so streaming and downloads always hit the network. Bump the cache name when shipping shell changes to force-refresh clients.
