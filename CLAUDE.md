...
- The service-worker cache bumps with each shell change (currently `v19`); `/static/` assets use **stale-while-revalidate** (serve cache, refetch in background) **and** are loaded with a `?v=<n>` query string from the HTML (bumped each shell change in every `*.html` + `sw.js`'s `ASSET_V`/`SHELL`) so a deploy serves a brand-new URL that can't hit a stale cache entry — CSS/JS deploys reach clients on the next load without a hard-refresh (`Ctrl+Shift+R` still makes it instant).
...
- **`main.py`** — FastAPI app (~2900 lines). All API routes (`/api/*`), lifespan hooks (DB init + scheduler start), the auth ASGI middleware, and static SPA mount. This is the single backend file for routes.
...
- Other pages: `index.html` (library, grouped by category with drag-&-drop reordering via `POST /api/podcasts/reorder`), `podcast.html` (single feed; includes a category dropdown), `inbox.html` ("Neuzugänge" — pending/queued items across all feeds via `GET /api/inbox`, with on-demand transcribe + `POST /api/feeds/check-all`; replaces "Entdecken" in the nav), `category.html` (one category's bundled podcasts/episodes/tags via `GET /api/categories/{id}`), `digest.html` (digest/"Redaktion" builder + recipes/scheduling — UI label is "Redaktion", routes/tables stay `digest`/`digests`; includes the "Freier Artikel" format with a free prompt + "KI wählt Folgen ↔ manuell" toggle), `digest-reader.html` (renders a generated digest article), `discover.html` (recommended feeds — still routed, linked from Abonnieren/Neuzugänge), `tags.html` (browse by topic tag), `topic.html` (single tag's episodes), `radar.html` ("Themen-Radar" topic overview), `search.html` ("Frag deine Bibliothek" — RAG Q&A / chat UI over `/api/ask` + `/api/chat`, with Markdown export + print via `POST /api/chat/export`), `statistics.html` ("Statistik", at `/statistik`, over `GET /api/statistics?days=` — feed-poll + per-episode processing stats from `feed_check_log`/`processing_log`), `login.html` (owner/guest/friend login — optional name + password), `settings.html` (API key, backend, intervals, access control + **friends** + **Zeitungen/Editionen** manager via `/api/paper-editions`, category management), `about.html` (incl. a **Unterstützen/Feedback** section: PayPal "Buy me a coffee" + a feedback form `POST /api/feedback`), `terms.html` (Nutzungsbedingungen, public at `/terms`). The homepage shows the editions as a prominent button row (`#editions-bar`, each opening its `last_digest_id`) above the auto-scrolling marquee **Aktuelles** ticker.
- **`sw.js`** — Service Worker (cache `podscribe-v19`); caches the app shell incl. `icons.js` for offline reading, and **skips** `/audio` and `/export` so streaming and downloads always hit the network. Bump the cache name when shipping shell changes to force-refresh clients.
