...
- The service-worker cache bumps with each shell change (currently `v13`); browsers auto-refresh, but a hard-refresh (`Ctrl+Shift+R`) speeds it up. **Shell asset versioning**: HTML pages are served via `main._html_page()` which stamps `?v=ASSET_VERSION` onto `app.js`/`icons.js`/`style.css` so fresh HTML never pairs with a stale SW-cached script. When shell JS/CSS changes, bump **both** `ASSET_VERSION` in `app/main.py` **and** the `sw.js` `CACHE` name + `SHELL` `?v=` query (same number).
...
- **`sw.js`** — Service Worker (cache `podscribe-v13`); caches the app shell incl. `icons.js` for offline reading, and **skips** `/audio`, `/export` and `/covers` so streaming, downloads and freshly-generated cover art always hit the network. Bump the cache name when shipping shell changes to force-refresh clients (and keep `SHELL` `?v=` + `main.ASSET_VERSION` in lockstep — see Synology notes).
