...
- The service-worker cache bumps with each shell change (currently `v13`); browsers auto-refresh, but a hard-refresh (`Ctrl+Shift+R`) speeds it up.
...
- **`sw.js`** — Service Worker (cache `podscribe-v13`); caches the app shell incl. `icons.js` for offline reading, and **skips** `/audio` and `/export` so streaming and downloads always hit the network. Bump the cache name when shipping shell changes to force-refresh clients.
