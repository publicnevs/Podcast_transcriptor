...
- The service-worker cache bumps with each shell change (currently `v18`); `/static/` assets use **stale-while-revalidate** (serve cache, refetch in background) so CSS/JS deploys self-heal on the next load, but a hard-refresh (`Ctrl+Shift+R`) makes it immediate.
...
- **`sw.js`** — Service Worker (cache `podscribe-v18`); caches the app shell incl. `icons.js` for offline reading, and **skips** `/audio` and `/export` so streaming and downloads always hit the network. Bump the cache name when shipping shell changes to force-refresh clients.
